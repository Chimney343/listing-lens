...
class GratkaSpider(scrapy.Spider):
    ...
    def parse_search(self, response):
        ...
        if links and current_page < self.area.max_pages:
            yield scrapy.Request(
                build_gratka_url(self.area, page=current_page + 1),
    ...
    def parse_detail(self, response):
        ...
        item["price_pln"] = self._parse_float(price_text.replace("zł", ""))
        ...
        item["rooms"] = self._safe_int(params.get("Liczba pokoi", "").split()[0] if params.get("Liczba pokoi") else None)
